...
class TimeSerie(object):
    ...
    def calculate_points(
            ...
            timewindow):
        """
        Calculate two arrays of points.
        The first one corresponds to past and actual points.
        The second one corresponds to future points.
        """

        result = [[]] * 2

        logger.debug("Calculate {0} points \
            (timeserie: {1}, timewindow: {2})".format(
            len(points),
            self,
            timewindow))
        ...
        for index in xrange(1, len(timeSteps)):
            ...
            result[0].append(aggregation_point)
        ...
        logger.debug(" + Nb points: {0}".format(len(result[0])))

        if self.forecast is not None and self.forecast.enable:

            fpoints = \
                self.forecast.calculate_points(result[0], timewindow, self)

            result.papend(fpoints)
