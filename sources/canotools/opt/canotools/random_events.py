import random, sys, json, cevent, time, datetime
...
for arg in sys.argv[1:]:
	...
	if t == None:
		raise Exception("Wrong argument %s, wait for arg=value where arg in '%s'" % (arg, ARGVS))
	else:
		globals()[name] = t[0](value)

start_time = now - period * count

print reduce(lambda x, y: x+y, ["%s (%s): %s\n " % (argv, globals()[argv], ARGVS[argv][1]) for argv in ARGVS])
...
def print_results(_from, to, results):
	...
	"""
	print "Interval: %s -> %s, results=%s" % (datetime.datetime.fromtimestamp(_from), datetime.datetime.fromtimestamp(to), results)

def get_event(result):
	"""
	Initialize an event with input couple of (timestamp, value)
	"""
	perf_data = {'metric': 'random_event', 'value': result[VALUE], 'unit': unit, 'type': "GAUGE", 'min': 0, 'max': 0, 'warn': None, 'crit': None}
...
def send_events(producer, results, sleep=0):
	...
	for index, result in enumerate(results):
		...
		if sleep: # print pourcentage of done sent events
			print '%s\% done. (%s/%s)' % ( (index * 100.0 / len(results)), index, len(results))
...
with Connection(hostname=host, userid=user, virtual_host=vhost) as conn:
	with producers[conn].acquire(block=True) as producer:		
		...
		# print new events
		print_results(now, now + duration, results)