...
Ext.define('widgets.eventcalendar.calendar_ajax_handler' , {
	...
	computeTagsFilter: function(from, to) {
		if(!!this.calendar.stacked_events_filter)
		{
			var filter = {};
			filter["$and"] = [];
			filter["$and"].push({"timestamp" : { "$gt" : from}});
			filter["$and"].push({"timestamp" : { "$lt" : to}});
			filter["$and"].push(JSON.parse(this.calendar.stacked_events_filter));
			return filter;
		}
	},
	...
	getEvents: function(start, end, callback)
	{
		...
		$.when.apply($, promises).then(function(schemas) {
			...
			if(promises.length === 1)
			{
				...
				events = ajaxHandler.format_events(data, arguments[2].eventsType);
