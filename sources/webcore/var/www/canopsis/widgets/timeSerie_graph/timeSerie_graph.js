...
Ext.define('widgets.timeSerie_graph.timeSerie_graph' , {
    extend: 'canopsis.lib.view.cperfstoreValueConsumerWidget',

    alias: 'widget.timeSerie_graph',
     ...
     * 12 monthly
     */
    forecast: function(y, alpha, beta, gamma, period, m) {

	if (y == null) {
	    return null;
	}
	
	var seasons = y.length / period;
	var a0 = calculateInitialLevel(y, period);
	var b0 = calculateInitialTrend(y, period);
	var initialSeasonalIndices = calculateSeasonalIndices(y, period, seasons);

	
	log.debug('Total observations: ' + y.length + ', Seasons ' + seasons + ', Periods ' + period);
	log.debug('Initial level value ' + a0 + ':');
	log.debug('Initial trend value ' + b0 + ':');
    
	log.debug('Seasonal Indices: ' + initialSeasonalIndices);

	var forecast = calculateHoltWinters(y, a0, b0, alpha, beta, gamma, initialSeasonalIndices, period, m, debug);
     ...
     */
    calculateHoltWinters: function(y, a0, b0, alpha,beta, gamma, initialSeasonalIndices, period, m, debug) {
	...
	for (var i = 2; i < y.length; i++) {
	    ...
	    log.debug('i = ' + i + ', y = ' + y[i] + ', S = ' + St[i] + ', Bt = ' + Bt[i] + ', It = ' + It[i] + ', F = ' + Ft[i]);	    
	}
     ...
     */
    calculateInitialLevel: function(y, period) {
	...
	**/
     ...
     */
    calculateInitialTrend: function(y, period){
     ...
     */
    calculateSeasonalIndices: function(y, period, seasons){

	var seasonalAverage = [];
	var seasonalIndices = [];

	var averagedObservations = [];
     ...
     * @param data
     */
    printArray: function(description, data){

	log.debug('Description : ' + description + ', data : ' + data);
